#!/bin/bash
#SBATCH --job-name=embeddings_tuned
#SBATCH --array=0-7
#SBATCH --time=06:00:00
#SBATCH --mem=200G
#SBATCH --cpus-per-task=4
#SBATCH --partition=pibu_el8
#SBATCH --output=/data/users/ltucker/influenzaData/H5N1_pipeline/log/embeddings_tuned_%A_%a.out
#SBATCH --error=/data/users/ltucker/influenzaData/H5N1_pipeline/log/embeddings_tuned_%A_%a.err
...
OUTPUT_DIR="/data/users/ltucker/influenzaData/H5N1_pipeline/output/famsa_per_segment_analysis/embeddings_tuned"
mkdir -p "$OUTPUT_DIR"
...
print(f"Distance matrix shape: {dist_matrix.shape} ({n_samples} samples)")
...
# --- t-SNE (tuned: higher perplexity for large datasets) ---
# Perplexity should scale with dataset size; cap at 200
perplexity = min(200, max(30, n_samples // 50))
print(f"[{seg_name}] Running t-SNE (perplexity={perplexity})...")
tsne = TSNE(
    n_components=2,
    metric="precomputed",
    init="random",
...
# --- UMAP (tuned: more neighbors for global structure) ---
n_neighbors = min(100, max(15, n_samples // 100))
print(f"[{seg_name}] Running UMAP (n_neighbors={n_neighbors}, min_dist=0.1)...")
...
# --- MDS (same as before, already looked good) ---
...
# --- PHATE (tuned: higher knn for smoother trajectories) ---
knn = min(30, max(15, n_samples // 200))
print(f"[{seg_name}] Running PHATE (knn={knn})...")
phate_op = phate.PHATE(
    n_components=2,
    knn=knn, # knn_dist="precomputed",
    decay=40,
    t="auto",
    n_landmark=2000,
    random_state=seed
...
print(f"[{seg_name}] Saved to {out_path}")
print(f"[{seg_name}] Output shape: {results.shape}")

# Log the parameters used
print(f"\n[{seg_name}] Parameters used:")
print(f"  t-SNE: perplexity={perplexity}, learning_rate={max(200, n_samples // 12)}, max_iter=2000")
print(f"  UMAP:  n_neighbors={n_neighbors}, min_dist=0.1, spread=1.0")
print(f"  MDS:   n_init=4")
print(f"  PHATE: knn={knn}, decay=40, t=auto, n_landmark=2000")
