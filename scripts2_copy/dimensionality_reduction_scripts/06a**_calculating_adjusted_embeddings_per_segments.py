...
OUTPUT_DIR="/data/users/ltucker/influenzaData/H5N1_pipeline/output/famsa_per_segment_analysis/embeddings_tuned_v2"
mkdir -p "$OUTPUT_DIR"
...
# --- UMAP (tuned v2: higher neighbors, slightly more spread) ---
n_neighbors = min(200, max(50, n_samples // 50))
print(f"[{seg_name}] Running UMAP (n_neighbors={n_neighbors}, min_dist=0.25)...")
...
# --- PHATE (tuned v2: knn=50 with precomputed distance) ---
print(f"[{seg_name}] Running PHATE (knn=50, knn_dist=precomputed)...")
...
print(f"  UMAP:  n_neighbors={n_neighbors}, min_dist=0.25, spread=1.0")
print(f"  MDS:   n_init=4")
print(f"  PHATE: knn=50, knn_dist=precomputed, decay=40, t=auto, n_landmark=2000")
