...
mat = df.iloc[:, 1:].values.astype(float)  # drop name column
n_min = min(mat.shape[0], mat.shape[1])
mat = mat[:n_min, :n_min]
upper_tri = mat[np.triu_indices(n_min, k=1)].reshape(-1, 1)
n_total_raw = len(upper_tri)
print(f'Upper triangle size: {n_total_raw} pairs')
print(f'Raw range: [{upper_tri.min():.6f}, {upper_tri.max():.6f}]')

# ── Filter: keep only distances in [0, 0.15] ───────────────────
mask = (upper_tri >= 0) & (upper_tri <= 0.15)
upper_tri = upper_tri[mask.flatten()].reshape(-1, 1)
n_total = len(upper_tri)
print(f'After filtering to [0, 0.15]: {n_total} pairs ({n_total/n_total_raw*100:.1f}% kept)')
print(f'Filtered range: [{upper_tri.min():.6f}, {upper_tri.max():.6f}]')
